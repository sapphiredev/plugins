...
**Plugin for <a href="https://github.com/sapphire-project/framework">@sapphire/framework</a> that makes it possible to create subcommands.**
...
This plugin enables you to create subcommands with the plugin's `Command` class. These support all features from Sapphire's base commands and even more: nested subcommands, aliases, parent precondition merging, et cetera.

## Features

-   Fully ready for TypeScript!
-   Includes ESM ready entrypoint
-   Indefinite subcommand nesting (e.g `!cmd sub1 sub2 ... subN`)
-   Subcommand precondition inheritance and merging
-   Subcommand aliases support
...
```typescript
import '@sapphire/plugin-subcommands/register-discordjs';
```

You will need to override the `events/command-handler/CorePrefixedMessage` event from Sapphire with your own that should emit `SubCommandsEvents.PreSubCommandRun` event instead of core `Events.PreCommandRun`. An example is as follows:

```typescript
export class UserEvent extends Event<Events.PrefixedMessage> {
	public run(message: Message, prefix: string) {
		// ... Similar code as in core class

		message.client.emit(SubCommandsEvents.PreSubCommandRun, { message, command, parameters, context: { commandName: name, prefix } });
	}
}
```

Import `Command` from plugin and extend with it:
...
## Sapphire subcommands Documentation
